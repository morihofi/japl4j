...
public enum PCAP_LINKTYPE {
    ...
    LINKTYPE_SLIP(8, "SLIP, with LINKTYPE_SLIP header."),
    ...
    LINKTYPE_ARCNET_LINUX(129, "ARCNET Data Packets, per RFC 1051 frames with variations"),
    ...
    LINKTYPE_LINUX_IRDA(144, "Linux-IrDA packets with LINKTYPE_LINUX_IRDA header"),
    LINKTYPE_IEEE802_11_AVS(163, "AVS monitor mode with 802.11 header"),
    LINKTYPE_BACNET_MS_TP(165, "BACnet MS/TP frames, per 9.3 MS/TP Frame Format ANSI 135"),
    LINKTYPE_PPP_PPPD(166, "PPP in HDLC-like encapsulation, like LINKTYPE_PPP_HDLC, with different stuffing"),
    ...
    PCAP_LINKTYPE(int value, String description) {
        this.value = value;
    ...
    public static PCAP_LINKTYPE findByValue(int value) {
        for (PCAP_LINKTYPE linkType : values()) {
            if (linkType.getValue() == value) {
                return linkType;
            }
        }
